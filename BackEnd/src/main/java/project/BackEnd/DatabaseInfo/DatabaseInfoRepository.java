...
public interface DatabaseInfoRepository extends JpaRepository<DatabaseInfo, Long> {
    ...
    @Query("SELECT DISTINCT db.databaseName, tb.tableName, fd.column_name FROM database_info db JOIN db.tables tb JOIN tb.ownershipDetails od JOIN od.user JOIN tb.fields fd WHERE od.user.username = :name ORDER BY 1, 2, 3")
